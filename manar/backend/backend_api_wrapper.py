...
async def startup_event():
    """Initialize ManaraAI system on startup"""
    global manara_ai
    try:
        print(" Initializing ManaraAI with FANAR API integration...")
        ...
        print(" ManaraAI system initialized successfully!")
        print(f" Vector database loaded with {manara_ai.rag_system.collection.count()} items")
        print(f" FANAR API status: {'Available' if manara_ai.rag_system.fanar_client.is_available else 'Fallback mode'}")
        
    except Exception as e:
        print(f" Error initializing ManaraAI: {e}")
...
@app.get("/")
async def root():
    fanar_status = "connected" if manara_ai and manara_ai.rag_system.fanar_client.is_available else "fallback mode"
...
async def health_check():
    fanar_health = "connected" if manara_ai and manara_ai.rag_system.fanar_client.is_available else "fallback"
    ...
    try:
        if not manara_ai:
            return ApiResponse(
                success=False,
                data={},
                message="ManaraAI system not initialized",
                request_id=request_id,
                timestamp=datetime.now().isoformat()
            )
        ...
        result = await manara_ai.rag_system.create_enhanced_day_plan(request.query, user_profile, parsed_preferences)
    ...
    try:
        if not manara_ai:
            return ApiResponse(
                ...
                message="ManaraAI system not available",
                request_id=request_id,
                timestamp=datetime.now().isoformat()
            )
...
async def fanar_status():
    ...
    if not manara_ai:
        return {"status": "not_initialized", "message": "ManaraAI system not initialized"}
    
    try:
        # Check FANAR API availability
        fanar_available = await manara_ai.rag_system.fanar_client._check_api_availability()
        
        return {
            "status": "healthy" if fanar_available else "fallback_mode",
            "fanar_api_available": fanar_available,
            "last_check": manara_ai.rag_system.fanar_client.last_check,
            "api_base": manara_ai.rag_system.fanar_client.base_url,
