...
app = FastAPI(
    title="manar API with FANAR Integration",
...
async def startup_event():
    """Initialize manarI system on startup"""
    global manar_ai
    try:
        print(" Initializing manarI with FANAR API integration...")
        ...
        print(" manarI system initialized successfully!")
        ...
        print(f" Error initializing manarI: {e}")
...
def _parse_planning_query(query: str) -> Dict:
    ...
    if 'extended day' in query_lower or '12 hour' in query_lower:
        parsed['duration'] = 12
        parsed['duration_text'] = '12 hours'
    elif 'full day' in query_lower or '6 hour' in query_lower:
        parsed['duration'] = 6
...
async def root():
    ...
    return {
        "message": "🇶🇦 manar Tourism API with FANAR Integration", 
        "status": "running", 
        "version": "1.0.0",
...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        if not manar_ai:
            return ApiResponse(
                success=False,
                data={"message": "manarI system not initialized. Please check the vector database setup."},
    ...
    try:
        if not manar_ai:
            return ApiResponse(
                success=False,
                data={},
                message="manarI system not initialized",
                request_id=request_id,
                timestamp=datetime.now().isoformat()
            )
    ...
    try:
        if not manar_ai:
            return ApiResponse(
                ...
                message="manarI system not available",
                request_id=request_id,
                timestamp=datetime.now().isoformat()
            )
...
async def fanar_status():
    ...
    if not manar_ai:
        return {"status": "not_initialized", "message": "manarI system not initialized"}
...
class QatarChatbot:
    ...
    def get_system_prompt(self):
        ...
        Format responses in a clear, SHORT, concise, and readable way.
...
if __name__ == "__main__":
    print(" Starting manar with FANAR API Integration...")
